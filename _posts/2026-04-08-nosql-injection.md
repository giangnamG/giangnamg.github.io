...
Nếu syntax injection nằm trong JavaScript expression context và `this` trỏ tới document hiện tại, có thể dùng `Object.keys(this)` để lấy danh sách field name của document. Phương pháp này cần boolean oracle: response phải khác nhau giữa điều kiện true và false.
...
### 4.3. Extract data bằng operator injection
