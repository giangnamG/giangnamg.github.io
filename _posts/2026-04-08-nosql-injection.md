...
Nếu response body bị làm giống nhau, vẫn có thể quan sát chênh lệch thời gian tự nhiên giữa các field-level condition trong luồng bypass. Ví dụ so sánh regex có khả năng khớp với regex không khớp trên field `password`:
...
Nếu môi trường tạo độ lệch thời gian ổn định giữa điều kiện đúng và sai, độ lệch đó có thể dùng như blind timing oracle. Đây chỉ là tín hiệu quan sát từ field-level condition, không phải JavaScript delay do payload điều khiển.
...
#### 4.1.9. Giới hạn của Parameter-scoped operator injection
...
Timing-based operator injection chỉ phù hợp khi top-level `$where` đi tới database và được evaluate như JavaScript. Nếu response body không phân biệt rõ điều kiện đúng/sai, thời gian phản hồi có thể được dùng làm oracle.

Trước hết cần đo baseline và dùng payload đối chứng. Ví dụ payload tạo delay không điều kiện:

```json
{"username":"admin","password":{"$ne":"invalid"},"$where":"function(){sleep(5000); return true;}"}
```

Nếu request này chậm hơn baseline một cách lặp lại, còn payload không delay thì không chậm, mới có cơ sở dùng timing. Một response chậm đơn lẻ không đủ để kết luận vì có thể do network hoặc tải hệ thống.

Sau đó đặt điều kiện vào `$where` để biến delay thành boolean oracle. Ví dụ kiểm tra ký tự đầu tiên của password:

```json
{"username":"admin","password":{"$ne":"invalid"},"$where":"function(){if(this.password[0]==='a'){sleep(5000)};return true;}"}
```

Nếu chỉ payload chứa ký tự đúng tạo độ trễ ổn định, có thể lặp lại theo từng vị trí và từng ký tự. Nếu độ trễ không ổn định, không nên dùng timing làm tín hiệu extract.
